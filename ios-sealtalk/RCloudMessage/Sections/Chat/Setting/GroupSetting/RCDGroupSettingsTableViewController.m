...
- (void)viewDidLoad {
    ...
    [self.navigationItem setLeftBarButtonItems:[RCDUIBarButtonItem getLeftBarButton:RCDLocalizedString(@"back") target:self action:@selector(backBarButtonItemClicked:)]];
...
- (void)choosePortrait {
    if ([[RCDGroupManager getGroupOwner:self.group.groupId] isEqualToString:[RCIM sharedRCIM].currentUserInfo.userId]) {
        [RCActionSheetView showActionSheetView:nil cellArray:@[RCDLocalizedString(@"take_picture"), RCDLocalizedString(@"my_album")] cancelTitle:RCDLocalizedString(@"cancel") selectedBlock:^(NSInteger index) {
            [self pushImagePickVC:(NSInteger)index];
        } cancelBlock:^{
...
- (void)cleanUpGroupMessagesRegularly {
    [RCActionSheetView showActionSheetView:nil cellArray:@[RCDLocalizedString(@"CleanUpGroupMessages36HoursAgo"), RCDLocalizedString(@"CleanUpGroupMessages3DaysAgo"), RCDLocalizedString(@"CleanUpGroupMessages7DaysAgo"), RCDLocalizedString(@"NeverClean")] cancelTitle:RCDLocalizedString(@"cancel") selectedBlock:^(NSInteger index) {
...
- (void)alertActionDidClickIndex:(NSInteger)index{
    RCDGroupMessageClearStatus status = RCDGroupMessageClearStatusUnknown;
    switch (index) {
        case 0:
            status = RCDGroupMessageClearStatusBefore36h;
            break;
        case 1:
            status = RCDGroupMessageClearStatusBefore3d;
            break;
        case 2:
            status = RCDGroupMessageClearStatusBefore7d;
            break;
        case 3:
            status = RCDGroupMessageClearStatusClose;
            break;
        default:
            break;
    }
    [RCDChatManager setGroupMessageClearStatus:status
