...
- (BOOL)handleWatchKitRequest {
    ...
    if ([query isEqualToString:WK_APP_COMMUNICATE_QUERY_UNREAD_COUNT]) {
        ...
        [messages enumerateObjectsUsingBlock:^(RCMessage *msg, NSUInteger idx, BOOL *stop) {
            if ([msg.content isKindOfClass:[RCImageMessage class]]) {
                RCImageMessage *imageMsg = (RCImageMessage *)msg.content;
                imageMsg.thumbnailImage = [self scaleImage:imageMsg.thumbnailImage maxWidth:80 round:NO];
                imageMsg.originalImage = [self scaleImage:imageMsg.originalImage maxWidth:140 round:NO];
            } else if ([msg.content isKindOfClass:[RCLocationMessage class]]) {
                RCLocationMessage *locationMsg = (RCLocationMessage *)msg.content;
                locationMsg.thumbnailImage = [self scaleImage:locationMsg.thumbnailImage maxWidth:80 round:NO];
            }
        }];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (RCUserInfo *userInfo in array) {
                NSURL *currentUrl = [containerUrl URLByAppendingPathComponent:userInfo.userId];
                if (![fileManager fileExistsAtPath:[currentUrl path]]) {
                    UIImage *image = [UIImage
                        imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:userInfo.portraitUri]]];
    ...
    } else if ([query isEqualToString:WK_APP_COMMUNICATE_REQUEST_CACHE_HEAD_ICON]) {
        ...
        NSURL *containerUrl =
            [fileManager containerURLForSecurityApplicationGroupIdentifier:[self.provider getAppGroups]];
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSURL *currentUrl = [containerUrl URLByAppendingPathComponent:targetId];
            if (![fileManager fileExistsAtPath:[currentUrl path]]) {
                if (conversationType.intValue == ConversationType_PUBLICSERVICE ||
                    ...
                    for (RCPublicServiceProfile *profile in publicServices) {
                        if ([profile.publicServiceId isEqualToString:targetId]) {
                            UIImage *image = [UIImage
                                imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:profile.portraitUrl]]];
                            NSLog(@"size is %f, %f", image.size.width, image.size.height);
                            if (image && [image size].width > 0) {
        ...
        NSString *targetID = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_TARGET_ID];
        NSNumber *conversationType = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_CONVERSATION_TYPE];
        NSData *contentData = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_CONTENT];
        RCMessageContent *content = [NSKeyedUnarchiver unarchiveObjectWithData:contentData];
        /*
        ...
        NSNumber *on = [NSNumber numberWithBool:[self.provider getNewMessageNotificationSound]];
        [self replyWKApp:on];
    } else if ([query isEqualToString:WK_APP_COMMUNICATE_SET_NOTIFY_SOUND]) {
        NSNumber *on = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_NEW_MESSAGE_SOUND];
        ...
        [self replyWKApp:nil];
    } else if ([query isEqualToString:WK_APP_COMMUNICATE_QUERY_CONNECTION_STATUS]) {

        [self replyWKApp:[NSNumber numberWithBool:[self.provider getLoginStatus]]];
    } else if ([query isEqualToString:WK_APP_COMMUNICATE_REQUEST_CLEAR_UNREAD_COUNT]) {
        NSString *targetID = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_TARGET_ID];
        NSNumber *conversationType = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_CONVERSATION_TYPE];
        BOOL result =
        ...
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:groups];
        [self replyWKApp:data];
    } else if ([query isEqualToString:WK_APP_COMMUNICATE_REQUEST_DOWNLOAD_IMAGE]) {
        NSString *targetId = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_TARGET_ID];
        NSNumber *conversationType = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_CONVERSATION_TYPE];
        NSString *imageUrl = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_IMAGE_URL];
